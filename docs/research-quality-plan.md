...
## 읽는 흐름 (v2.1, 2026-09-26)

- 첫 방문: 탭 아래 "읽는 법" 안내(한 번 닫으면 다시 안 뜸).
- 재방문: 마지막 방문일을 브라우저에 기억. 그 이후 날짜의 항목에 "새로", 탭 이름 옆에 새 항목 수. 30분 안의 새로고침은 같은 기준 유지. 판단 단위는 날짜라서 탭 숫자와 카드 표시가 항상 일치한다.
- 읽음: 원문 링크, 이미지, 판독 펼치기를 누르면 "읽음"(최근 4,000개). 모두 이 브라우저 localStorage에만 저장. 저장이 막힌 환경에서도 페이지는 정상 동작.
- 탭 기본 모드는 "훑어보기"(작은 썸네일, 제목, 가져갈 한 수). "판독 펼치기"로 새로움, 작동 원리, 근거, 한계. "자세히"로 전체 펼침 모드 전환(기억됨).
- 날짜 머리는 스크롤 중 상단에 고정.
- 발견: 카드의 "비슷한 것"과 "# 신호"는 아카이브 전체 의미 검색으로 연결(자기 자신은 결과에서 제외). 결과 화면의 "돌아가기"와 브라우저 뒤로가기 모두 원래 탭의 읽던 위치로 복귀.
- 탭 전환 시 탭별 스크롤 위치 기억. 같은 탭을 다시 누르면 맨 위로.
