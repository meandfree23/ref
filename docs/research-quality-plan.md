...
- 워크플로는 07:10과 11:10 KST 두 번 예약된다. GitHub cron 지연 대비. 모든 단계는 멱등이라 두 번 돌아도 안전하다.
- 수집(`recover:daily`)이 실패해도 판독·배포는 계속된다. 수집기는 실패 시 스냅샷을 복원하므로 기존 데이터로 진행한다.
- 커밋 직전 `scripts/verify-site-data.mjs`가 산출물을 검사한다: JSON 유효성, 탭별 항목 수·판독 수가 직전 커밋 대비 3% 이상 줄지 않았는지, 최신 날짜가 뒤로 가지 않았는지, 하루 15개 초과·중복 id 없음. 실패하면 아무것도 푸시하지 않는다.
- `scripts/dedupe-archives.mjs`가 교차 날짜 중복 URL을 제거한다(먼저 보관된 쪽 유지, 판독 결과는 승계).
- 게이트 규칙을 바꿀 때는 반드시 `npm run quality:check`를 로컬에서 먼저 통과시킨다(고정 샘플 테스트가 있다).
- 사이트 상단은 마지막 갱신이 40시간을 넘으면 빨간 경고를 띄운다.
- 소스 강등은 30일 동안 다시 보이지 않으면 자동 해제된다(영구 배제 방지).
